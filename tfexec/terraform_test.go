...
func TestCheckpointDisablePropagation(t *testing.T) {
	...
	expected := []string{"CHECKPOINT_DISABLE=1", "FOOBAR=1", "TF_INPUT=0", "TF_IN_AUTOMATION=1", "TF_LOG=", "TF_LOG_PATH="}
	s := initCmd.Env
	sort.Strings(s)
	actual := s
	...
	initCmd = tf.initCmd(context.Background())
	expected = []string{"CHECKPOINT_DISABLE=", "FOOBAR=1", "TF_INPUT=0", "TF_IN_AUTOMATION=1", "TF_LOG=", "TF_LOG_PATH="}
	s = initCmd.Env
	sort.Strings(s)
	actual = s
